...
class SuperMarioBrosEnv(NESEnv):
    ...
    def _x_position(self):
        ...
        curr_pos = self.ram[0x86]
        # raise ValueError(f"curr_pos: {curr_pos}, {self.ram[0x6d]}, {0x100}")
        curr_page = np.uint8(np.uint64(self.ram[0x6D]) * 0x100 % 256)
        return curr_page + curr_pos
    ...
    def _left_x_position(self):
        ...
        curr_pos = np.uint64(self.ram[0x86])
        left_pos = np.uint64(self.ram[0x071C])
        new_pos = (curr_pos - left_pos) % 256
        return np.uint8(new_pos)
    ...
    def _x_reward(self):
        ...
        _reward = float(self._x_position) - float(self._x_position_last)
