...
def test_atari_preprocessing_grayscale(env_fn):
    ...
    assert numpy.allclose(obs3, cv2.resize(obs1, (84, 84), interpolation=cv2.INTER_AREA))
    ...
    assert numpy.allclose(obs2[10:38], obs3_gray[10:38])
    # the paddle also do not render quite the same
    assert numpy.allclose(obs2[44:], obs3_gray[44:])
