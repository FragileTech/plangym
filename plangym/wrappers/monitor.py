...
from gym import error, logger, version, Wrapper
...
class Monitor(Wrapper):
    ...
    def _start(
        ...
        Args:
            ...
            video_callable (Optional[function, False]): function that takes in \
                          the index of the episode and outputs a boolean, \
                          indicating whether we should record a video on this \
                          episode. The default (for video_callable is None) is \
                          to take perfect cubes, capped at 1000. False disables \
                          video recording.
            ...
            write_upon_reset (bool): Write the manifest file on each reset. \
                                    (This is currently a JSON file, so writing \
                                    it is somewhat expensive.)
            uid (Optional[str]): A unique id used as part of the suffix for the \
                                file. By default, uses os.getpid().
            mode: (['evaluation', 'training']) Whether this is an evaluation \
                or training episode.
        """
    ...
    def close(self):
        ...
        logger.info(
            "Finished writing results. You can upload them to the "
            "scoreboard via gym.upload(%r)",
            self.directory,
        )
    ...
    def _after_step(self, observation, reward, done, info):
        ...
        if done and self.env_semantics_autoreset:
            # For envs with BlockingReset wrapping VNCEnv, this observation will
            # be the first one of the new episode
...
def merge_stats_files(stats_files):
    ...
    idxs = numpy.argsort(timestamps)
    timestamps = numpy.array(timestamps)[idxs].tolist()
    episode_lengths = numpy.array(episode_lengths)[idxs].tolist()
    episode_rewards = numpy.array(episode_rewards)[idxs].tolist()
    data_sources = numpy.array(data_sources)[idxs].tolist()

    if episode_types:
        episode_types = numpy.array(episode_types)[idxs].tolist()
    else:
        episode_types = None
